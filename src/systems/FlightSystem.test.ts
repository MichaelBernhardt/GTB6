...
function takeoff(until = 120): PlaneState {
  ...
  fly(state, { throttle: 1, steer: 0, pitch: 0 }, 8, 0); // ground roll to speed
  expect(state.grounded).toBe(true);
  let step = stepPlane(state, { throttle: 1, steer: 0, pitch: 1 }, DT, 0, 0);
  for (let tick = 0; tick < 60 * 60 && step.y < until; tick++) step = stepPlane(state, { throttle: 1, steer: 0, pitch: 1 }, DT, step.y, 0);
  expect(state.grounded).toBe(false);
  expect(step.y).toBeGreaterThanOrEqual(until);
  // settle level at cruise
  for (let tick = 0; tick < 120; tick++) step = stepPlane(state, { throttle: 1, steer: 0, pitch: 0 }, DT, step.y, 0);
  return state;
}
...
describe('ground roll', () => {
  it('throttle winds up and the roll accelerates, staying on the tar without a pull-up', () => {
    const state = createPlaneState(0);
    const step = fly(state, { throttle: 1, steer: 0, pitch: 0 }, 10, 0);
  ...
  it('steers on the nosewheel', () => {
    const state = createPlaneState(0);
    fly(state, { throttle: 1, steer: 1, pitch: 0 }, 4, 0);
  ...
  it('will not rotate below flying speed', () => {
    const state = createPlaneState(0);
    fly(state, { throttle: 0.001, steer: 0, pitch: 1 }, 2, 0);
    state.speed = PLANE_ROTATE_SPEED - 5;
    stepPlane(state, { throttle: 0, steer: 0, pitch: 1 }, DT, 0, 0);
    expect(state.grounded).toBe(true);
  });
  ...
  it('brakes to a stop with S at idle throttle', () => {
    const state = createPlaneState(0);
    state.speed = 20;
    fly(state, { throttle: -1, steer: 0, pitch: 0 }, 4, 0);
...
describe('takeoff and cruise', () => {
  ...
  it('auto-levels hands-off: pitch trims back near zero and altitude holds', () => {
    const state = takeoff();
    let step = stepPlane(state, { throttle: 1, steer: 0, pitch: 0 }, DT, 300, 0);
    for (let tick = 0; tick < 300; tick++) step = stepPlane(state, { throttle: 1, steer: 0, pitch: 0 }, DT, step.y, 0);
  ...
  it('banks into a turn and rolls back level when the stick is released', () => {
    ...
    fly(state, { throttle: 1, steer: 1, pitch: 0 }, 3, 300);
    expect(state.roll).toBeGreaterThan(0.5);
    expect(state.heading).toBeGreaterThan(before + 0.8);
    fly(state, { throttle: 1, steer: 0, pitch: 0 }, 3, 300);
    expect(Math.abs(state.roll)).toBeLessThan(0.05);
  });

  it('tops out at the ceiling: a held climb cannot punch far past it', () => {
    const state = takeoff();
    let step = stepPlane(state, { throttle: 1, steer: 0, pitch: 1 }, DT, 550, 0);
    let peak = step.y;
    for (let tick = 0; tick < 60 * 40; tick++) { step = stepPlane(state, { throttle: 1, steer: 0, pitch: 1 }, DT, step.y, 0); peak = Math.max(peak, step.y); }
...
describe('stall', () => {
  it('closing the throttle bleeds airspeed until the plane stalls and comes down', () => {
    const state = takeoff();
    const cut: PlaneStick = { throttle: -1, steer: 0, pitch: 0 };
  ...
  it('holding the stick back cannot out-muscle a deep stall', () => {
    ...
    fly(state, { throttle: 0, steer: 0, pitch: 1 }, 3, 400);
...
describe('landing', () => {
  it('a slow, shallow touchdown rolls out instead of wrecking', () => {
    ...
    let step = stepPlane(state, { throttle: 0.4, steer: 0, pitch: 0 }, DT, 6, 0);
    for (let tick = 0; tick < 60 * 20 && !step.landed; tick++) step = stepPlane(state, { throttle: 0.4, steer: 0, pitch: -0.06 < state.pitch ? -0.2 : 0 }, DT, step.y, 0);
  ...
  it('a steep dive into the ground is a crash', () => {
    ...
    const step = stepPlane(state, { throttle: 1, steer: 0, pitch: -1 }, DT, 0.4, 0);
    expect(step.landed).toBe(true);
