...
export class MemoryProfileStore {
  ...
  async load(token, name) {
    ...
    const profile = { name, kills: 0, deaths: 0, runs: 0 };
    this.profiles.set(tokenHash(nextToken), profile);
    return { token: nextToken, profile: { ...profile } };
  }
  async save(token, profile) { this.profiles.set(tokenHash(token), { runs: 0, ...profile }); }
  async close() {}
}
...
export class PostgresProfileStore {
  ...
  async init() {
    ...
    await this.pool.query('ALTER TABLE multiplayer_profiles ADD COLUMN IF NOT EXISTS runs INTEGER NOT NULL DEFAULT 0');
  }
  async load(token, name) {
    if (token) {
      const result = await this.pool.query('SELECT display_name, kills, deaths, runs FROM multiplayer_profiles WHERE token_hash = $1', [tokenHash(token)]);
      if (result.rowCount) {
        ...
        return { token, profile: { name, kills: row.kills, deaths: row.deaths, runs: row.runs ?? 0 } };
      }
    }
    const nextToken = createToken();
    await this.pool.query('INSERT INTO multiplayer_profiles (token_hash, display_name) VALUES ($1, $2)', [tokenHash(nextToken), name]);
    return { token: nextToken, profile: { name, kills: 0, deaths: 0, runs: 0 } };
  }
  async save(token, profile) {
    await this.pool.query('UPDATE multiplayer_profiles SET display_name = $2, kills = $3, deaths = $4, runs = $5, updated_at = NOW() WHERE token_hash = $1', [tokenHash(token), profile.name, profile.kills, profile.deaths, profile.runs ?? 0]);
  }
