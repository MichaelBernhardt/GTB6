 ...
 * old scale, 3.0 at the 18000u parity scale) so the zones still cover the SAME real CBD patch at
 * any TARGET_SIZE — the roadside-building pitch scales with ROAD_SAMPLE_SPACING in the same
 * proportion, so real building density (and thus the ~340 budget) is preserved.
 */
 ...
 * scaled by P so it stays that same real patch (~1080u at the 18000u parity footprint).
 */
