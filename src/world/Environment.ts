...
export function buildEnvironment(scene: THREE.Scene, quality: 'low' | 'medium' | 'high'): EnvironmentHandle {
  ...
  scene.fog = new THREE.FogExp2(0xc4b48c, quality === 'low' ? 0.00032 : 0.00025); // tuned for the 36000u map with chunk culling (~2500u radius) and camera far 8000: the CBD skyline still reads out to ~2200u, the chunk boundary sits in a solid haze band, and the horizon is near-opaque by the far plane so it reads hazy, never clipped
