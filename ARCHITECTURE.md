...
- `UrbanInfrastructure` owns the repeated streetscape layer: instanced broadleaf and palm vegetation, shrubs, road-facing outer-curb lighting, furniture, transit shelters, regulatory and street-name signs, plus independently phased animated signal heads. `City` exposes road-surface clearance and dedicated roadside points so props cannot occupy carriageways or cross-road overlaps; curbs are clipped against every road and tactile paving marks signalized corners. Two anchor kinds exist and the difference is deliberate: **amenities** (benches, bins, cabinets, vegetation) are strided over the verge `roadsidePoints`, which is a lottery and suits a bench; **infrastructure that must be within reach of everywhere** — streetlamps, fire hydrants — is anchored by ARC LENGTH along each street (`buildStreetlampPoints`, `buildHydrantStations`), which is a coverage guarantee per road and cannot be reshuffled by re-tuning a modulus. Every prop is grounded on `City.surfaceHeightAt` — the surface actually drawn at that spot, and the same one the player, the peds and each prop's own collider base use — never on "pavement height everywhere".
