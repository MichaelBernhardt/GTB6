...
export class MenuView {
  ...
  cheats(weapons: CheatWeaponEntry[], cheats: CheatSettings, actions: { weapon: (id: CheatWeaponEntry['id']) => void; maxAmmo: () => void; toggle: (value: Partial<CheatSettings>) => void; back: () => void }): void {
    ...
    for (const key of ['fastRun', 'bigJump', 'invulnerable', 'teflon'] as const) this.root.querySelector<HTMLInputElement>(`[data-cheat="${key}"]`)?.addEventListener('change', (event) => actions.toggle({ [key]: (event.target as HTMLInputElement).checked }));
  }
