...
describe('Jozi-by-the-Sea coast', () => {
  ...
  it('coastline forms a continuous south-to-north boundary along the west edge', () => {
    ...
    // West of the corridor band (the odd shoreline wobble may lap slightly over its edge).
    expect(Math.max(...coast.coastline.map((point) => point[0]))).toBeLessThan(coast.corridor.westX + 80);
  });
...
describe('rural corridor', () => {
  it('is a real drive: within the owner range, crossed by both creative links', () => {
    expect(map.stats.corridorWidthUnits).toBeGreaterThanOrEqual(600);
    expect(map.stats.corridorWidthUnits).toBeLessThanOrEqual(950);
  ...
  it('has farmland fields, farm buildings and dirt tracks inside the corridor band', () => {
    ...
    for (const field of fields) {
      for (const point of field.points) {
        expect(point[0]).toBeGreaterThan(coast.corridor.westX - 120);
        expect(point[0]).toBeLessThan(coast.corridor.eastX + 120);
      }
    }
