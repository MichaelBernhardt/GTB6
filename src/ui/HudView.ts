...
export class HudView {
  ...
  private passedName: HTMLElement;
  private passedItems: HTMLElement;
  ...
  update(state: HudState): void {
    ...
    if (state.missionPassed) {
      setText(this.passedName, state.missionPassed.name);
      const key = state.missionPassed.items.join('|');
      if (this.passedItemsKey !== key) { this.passedItemsKey = key; this.passedItems.innerHTML = state.missionPassed.items.map((item: string) => `<li>${item}</li>`).join(''); }
    }
