...
export class SafehouseSystem {
  ...
  constructor(scene: THREE.Scene, city: City) {
    ...
    this.buildFlat(city);
    // Sit the gate pad and the respawn point on the ground too — the world has real terrain elevation, so a
    ...
    for (const place of SAFEHOUSES) {
      ...
      place.spawn[1] = city.surfaceHeightAt(place.spawn[0], place.spawn[2]) + 1;
