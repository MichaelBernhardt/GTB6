...
describe('CBD test-building zones (data-driven massing scope)', () => {
  it('anchors every zone inside the world and around the CBD', () => {
    ...
    for (const zone of TEST_BUILDING_ZONES) {
      ...
      expect(zone.radius, `${zone.name} radius sane`).toBeGreaterThan(80);
      expect(zone.radius, `${zone.name} radius sane`).toBeLessThan(700);
      // Every zone sits in/near the CBD district so this build populates the inner city, not the veld.
      expect(Math.hypot(zone.x - CBD_CENTER.x, zone.z - CBD_CENTER.z), `${zone.name} near CBD`).toBeLessThan(CBD_CENTER.radius + 200);
    }
  });
  ...
  it('leaves the far corners of the 6000u map as bare roads', () => {
