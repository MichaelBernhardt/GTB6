export class InputManager {
  ...
  constructor(private element: HTMLElement) {
    ...
    window.addEventListener('mousedown', (event) => { if (event.button === 0) { this.firing = true; this.firePressed = true; } });
    window.addEventListener('mouseup', (event) => { if (event.button === 0) this.firing = false; });
    window.addEventListener('wheel', (event) => { if (document.pointerLockElement === this.element) this.wheel += Math.sign(event.deltaY); }, { passive: true });
  ...
  get aiming(): boolean { return this.held.has('ControlLeft') || this.held.has('ControlRight'); }
