...
export class PopulationSystem {
  ...
  spawnAmbientPedestrian(x: number, z: number): Pedestrian {
    const ped = new Pedestrian(this.scene, new THREE.Vector3(x, 0, z), this.ambientSerial++);
    ped.pickDestination(this.city.sidewalkPoints); this.pedestrians.push(ped); return ped;
  }

  /** Lifecycle spawn: one AI-driven car dropped on a vetted lane node and routed immediately. */
  spawnTrafficVehicle(x: number, z: number): Vehicle {
    const kinds: VehicleKind[] = ['compact', 'taxi', 'sport', 'van'];
