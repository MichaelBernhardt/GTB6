...
export class Game {
  ...
  private updateCoverState(dt: number): CoverPose | undefined {
    ...
    if (this.settings.cameraViewFoot === 0) { this.cover = undefined; this.coverAvailable = false; return undefined; }
    if (!this.cover) {
  ...
  private die(): void {
    ...
    this.cover = undefined; this.mode = 'dead'; this.deathTimer = 3; this.audio.setEngine(false); this.audio.setSiren(false); this.audio.setFire(false); this.audio.stopRadio(); this.closeWeaponWheel(); this.ui.notify('EISH', 'You got klapped. An ambulance is coming just now. Press E after respawning to restart the job.', false); document.exitPointerLock();
  }
